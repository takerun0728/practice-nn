...
class IrisClassificationNetwork(AbstractNetwork):
    def __init__(self, mid_neuron_num, activate_func='relu', wb_width=0.1, optimizer='momentum', opt_params={'eta':0.001, 'rho':0.9, 'alpha':0.9}, lam=0.2, dropout=0.0):
        super().__init__()
        self.layers = [Layer(4, mid_neuron_num, activate_func=activate_func, wb_width=wb_width, optimizer=optimizer, opt_params=opt_params, lam=lam)]
        ...
        self.layers.append(Dropout(dropout))
        self.layers.append(OutputLayer(mid_neuron_num, 3, activate_func='softmax', opt_params=opt_params, lam=lam))

if __name__ == '__main__':
    network = IrisClassificationNetwork(MID_NEURON_NUM)
